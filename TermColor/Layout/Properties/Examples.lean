...
theorem neutral_chess_width
    : charWidth (Char.ofNat 0x265C) = 1
    := by
  decide

theorem ambiguous_spade_width
    : charWidth (Char.ofNat 0x2660) = 1
    := by
  decide

theorem neutral_arrow_width
    : charWidth (Char.ofNat 0x27B5) = 1
    := by
  decide
...
theorem ansi_style_does_not_change_width
    : Text.width (Text.styled "warning" Style.bold) = 7
    := by
  decide
...
theorem columns_example
    : (columns [4, 4] 1 [Text.plain "a", Text.plain "b"]).plainText = "a    b   "
    := by
  decide

theorem columns_wrap_example
    : (columns [4, 4] 1 [Text.plain "abcde", Text.plain "x"]).plainText = "abcd x   \ne        "
    := by
  decide
